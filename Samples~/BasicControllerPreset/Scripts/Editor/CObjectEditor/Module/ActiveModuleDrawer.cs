...
public class ActiveModuleDrawer : ModuleDrawer
{
    ...
    void SortTheList(SerializedProperty property)
    {
        if(property.isArray)
        {
            ...
            for (j=1; j<N; j++) {
                for (i=j; i>0 && (property.GetArrayElementAtIndex(i).managedReferenceValue as ActiveRoutine).priorityID 
                    < (property.GetArrayElementAtIndex(i-1).managedReferenceValue as ActiveRoutine).priorityID; i--) {
